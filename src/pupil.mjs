...
const getSinglePupil = function (pixels, width, height) {
  ...
  for (
    var halfWidth = Math.floor(height / 10.0);
    halfWidth < width / 2;
    halfWidth++
  ) {
    ...
    for (var x = halfWidth; x < width - offset; x++) {
      for (var y = halfWidth; y < height - offset; y++) {
        ...
        var irisArea =
          summedAreaTable[x + offset][y + offset] +
          summedAreaTable[x + offset - halfWidth][y + offset - halfWidth] -
          summedAreaTable[x + offset][y + offset - halfWidth] -
          summedAreaTable[x + offset - halfWidth][y + offset];
        var avgScore =
          (1.0 * irisArea) / ((halfWidth + 1) * (halfWidth + 1)) + 1;
        //summation area table again
        var scleraIrisArea =
          1.0 * summedAreaTable[width - 1 - offset][y + offset] +
          summedAreaTable[0 + offset][y + offset - halfWidth] -
          summedAreaTable[0 + offset][y + offset] -
          summedAreaTable[width - 1 - offset][y + offset - halfWidth] -
          irisArea;
  ...
  if (!eyesObj.left.blink) {
    eyesObj.left.pupil = getSinglePupil(
      Array.prototype.slice.call(
        webgazer.util.grayscale(
          eyesObj.left.patch.data,
          eyesObj.left.width,
          eyesObj.left.height
        )
      ),
